...
public class Application : Gtk.Application {
    ...
    private string? call_sytem_function (string[] args) {
        try {
    ...
    private void create_cert_bash() {
        var path_to_conf = GLib.Environment.get_user_config_dir() + "/nuntius/";
        var file = File.new_for_path (path_to_conf + "nuntius.pem");
        if (file.query_exists ()) {
            string[] spawn_args = {"./createcert.sh"};
            call_sytem_function(spawn_args);
            cert_created = true;
        }
        try {
            cert = new TlsCertificate.from_files (path_to_conf + "nuntius.pem", path_to_conf + "nuntius.key");
        } catch (Error e) {
    ...
    private string get_fingerprint() {
        string path_to_pem = GLib.Environment.get_user_config_dir() + "/nuntius/nuntius.pem";
        string[] spawn_args = {"openssl", "x509", "-in", path_to_pem, "-fingerprint", "-sha1"};
        string result = call_sytem_function(spawn_args); 
    ...
    private string get_hostname() {
        ...
        string hostname = call_sytem_function(spawn_args); 
        return hostname;
    }
